...
class Cocokeypoints(Dataset):
    ...
    def add_neck(self, meta):
        ...
        # print(type(meta['joint_self'])) #numpy.array
    ...
    # sierkinhane's crop and padding
    def sier_resize_pad(self, meta, img, mask_miss):

        h, w = img.shape[:2]
        max_ = np.maximum(h, w)
        to_size = self.params_transform['crop_size_y']
        top, left = 0, 0
        bottom, right = max_ - h, max_ - w
        img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(128, 128, 128))
        mask_miss = cv2.copyMakeBorder(mask_miss, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(255, 255, 255))
        img = cv2.resize(img, (0,0), fx=to_size/max_, fy=to_size/max_, interpolation=cv2.INTER_CUBIC)
        mask_miss = cv2.resize(mask_miss, (0,0), fx=to_size/max_, fy=to_size/max_, interpolation=cv2.INTER_CUBIC)
        # modify meta data
        meta['objpos'] = meta['objpos'] * (to_size/max_)

        for i in range(18):
            meta['joint_self'][i, :] = meta['joint_self'][i, :] * (to_size/max_)

        for j in range(meta['numOtherPeople']):

            meta['objpos_other'][j, :] = meta['objpos_other'][j, :] * (to_size/max_)

            for i in range(18):
                meta['joint_others'][j, i, :] = meta['joint_others'][j, i, :] * (to_size/max_)

        return meta, img, mask_miss

    def __getitem__(self, index):
        idx = self.index_list[index]
        # print(os.path.join(self.root, self.data[idx]['img_paths']))
        img = cv2.imread(os.path.join(self.root, self.data[idx]['img_paths']))
        # print(self.data[idx]['img_paths'])
...
#        print self.root + 'mask2014/val2014_mask_miss_' + img_idx + 'png'
        ...
        meta_data, img, mask_miss = aug_croppad(
            meta_data, img, mask_miss, self.params_transform)
        # else:
        #     meta_data, img, mask_miss = self.sier_resize_pad(
        ...
        elif self.preprocess == 'dense_pose':
